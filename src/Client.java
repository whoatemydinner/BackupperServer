        ...
        while(true) {
            ...
            if(command.equals(BackupProt.LOGIN)) {
                ...
                if(!(server.loginExists(login))) {
                    ...
                    send(BackupProt.LOGGEDIN + getUserFiles()); // serwer wysyla komende LOGGEDIN
                    //wraz ze wszystkimi plikami, ktore znajduja sie w folderze uzytkownika
                }
                
            } else if(command.equals(BackupProt.CREATE)) {
                ...
                if(server.loginExists(login)) {
                    ...
                    send(BackupProt.CREATED);
                }
                
            } else if(command.equals(BackupProt.ADD_FILE)) {
                ...
                if(!alreadyExists(fileName)) {
                    send(BackupProt.ADD_ACCEPTED + " " + fileName + " " + filePath + " " + fileSize);
                    
                } else {
                    send(BackupProt.FILE_EXISTS);
                }
                
            } else if(command.equals(BackupProt.ADD_START)) {
                ...
                try {
                    ...
                    send(BackupProt.ADD_SUCCESS + getUserFiles());
                } catch (IOException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                }
                
            } else if(command.equals(BackupProt.DELETE_FILE)) {
                
                String fileName = st.nextToken();
                if(alreadyExists(fileName)) {
                    deleteFile(fileName);
                    send(BackupProt.DELETE_SUCCESS+ getUserFiles());
                    
                } else {
                    send(BackupProt.NOT_FOUND);
                }
                
            } else if(command.equals(BackupProt.GET_FILE)) {
                String fileName = st.nextToken();
                String fileSize = st.nextToken();
                if(alreadyExists(fileName)) {
                    try {
                        send(BackupProt.SEND_START + " " + fileName + " " + fileSize);
                        sendFile(fileName);
            ...
            } else if(command.equals(BackupProt.NULL_COMMAND)) {
                send(BackupProt.NULL_COMMAND); // tylko do testow. potem usunac
                break;
            } else if(command.equals(BackupProt.LOGOUT)) {
                send(BackupProt.LOGGEDOUT);
                break;
            } else if(command.equals(BackupProt.STOPPED)) {
                break;
            }
